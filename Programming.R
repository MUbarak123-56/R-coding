...
## These three functions are used to display diagnostic information
## Messaging is used for displaying messages associated with the codes
...
## Warning is used to display warning associated with passing the wrong values
...
## Errors are used to stop the code from compiling due to coding error
...
## Magic: This used to convert strings into code

atan(c(-Inf, -1, 0, 1, Inf))
(quoted_r_code <- quote(atan(c(-Inf, -1, 0, 1, Inf))))
class(quoted_r_code)
eval(quoted_r_code) ## used to execute quoted code
as.list(quoted_r_code) 
vapply(list(`+`, `if`, `for`, `<-`, `[`, `[[`),  is.function,  logical(1)) 

parsed_r_code <- parse(text = "atan(c(-Inf, -1, 0, 1, Inf))") ## parse returns and expression which is a list of calls
class(parsed_r_code) 
eval(parsed_r_code)
...
## S3 CLASSES
print ## function (x, ...) ## UseMethod("print") ## <bytecode: 0x0000000018fad228> ## <environment: namespace:base> 
today <- Sys.Date() 
print(today) 
print.Date
head(methods(print)) 
...
my_class_generator <- setRefClass(  
  ...
  methods = list(    #functions to operate on that data go here    
    initialize = function(...)    
    {    
...
three_d_point_generator <- setRefClass(  
  ...
  contains = "point",         #this line lets us inherit  the class "point"
  methods  = list(initialize = function(x, y, z)    
  {      
    "Assign x and y upon object creation."      
    x <<- x      
    y <<- y      
