...
paste(c("red", "yellow"), "lorry") ## used to combine elements of a character vector with a string
paste(c("red", "yellow"), "lorry", sep = "-") ## applies a separation sign
paste(c("red", "yellow"), "lorry", collapse = ", ") ## separates all the elements but puts them under one single string
paste0(c("red", "yellow"), "lorry") 

x <- (1:15) ^ 2 
toString(x) ## works like paste
toString(x, width = 40)

cat(c("red", "yellow"), "lorry")  ## works like paste but adds an extra element

x <- c(  "I", "saw", "a", "saw", "that", "could", "out",  "saw", "any", "other", "saw", "I", "ever", "saw" ) 
noquote(x) ## used to remove quotes from a string vector
...
## SPECIAL CHARACTERS
# backslash is used for character symbols, cat is to pass these characters instead of print
cat("foo\tbar", fill = TRUE)
cat("foo\nbar", fill = TRUE) 
Cat("foo\0bar", fill = TRUE)  #this throws an error 
cat("foo\\bar", fill = TRUE) ## backslash needs to be doubled to prevent confusion
cat("foo\"bar", fill = TRUE) 
cat('foo\'bar', fill = TRUE) 
cat("foo'bar", fill = TRUE) ## here, backslash doesn't need to be used since it's a single quote within a double quote
cat('foo"bar', fill = TRUE) ## here, backslash doesn't need to be used since it's a double quote within a single quote
cat("\a") 
alarm()
...
# 1
pi <- 22/7
formatC(pi, digits = 16)  #16 sig figs 
# 2
